...
async function markNotificationRead(
  ...
  if (error) {
    console.error(
      'Could not mark manager notification as read:',
      error,
    );
  }
...
async function openNotification(
  ...
  if (error) {
    console.error(
      'Could not mark opened manager notification as read:',
      error,
    );
  }
...
export default async function ManagerNotificationsPage() {
  ...
  if (!auth) {
    redirect(
      '/member-access?returnTo=/platform/manager/notifications',
    );
  }
  ...
  if (error) {
    console.error(
      'Could not load manager notifications:',
      error,
    );
  }
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-lg leading-8 text-[#657B6C]">
                ...
                educator account changes,
                Family Voice prompts,
                reports and Regulator
                Champions service updates
                will appear here until
                you have read them.
              </p>
            </div>
