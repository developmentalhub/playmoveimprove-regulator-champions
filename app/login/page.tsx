...
function MemberAccessContent() {
  ...
  const [isCheckingExistingAccess, setIsCheckingExistingAccess] = useState(true);
  ...
  useEffect(() => {
    ...
    const checkExistingAccess = async () => {
      try {
        ...
        const result = (await response.json()) as AccessStatusResponse;
        ...
        console.error('Existing member access check failed:', error);
      } finally {
        if (isMounted) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!cleanCode) {
      setErrorMessage('Please enter your service access code.');
      return;
    }
    ...
    try {
      ...
      const result = (await response.json()) as ValidateAccessResponse;
      ...
      console.error('Member access submission failed:', error);
      setErrorMessage('We could not check your access code. Please try again.');
    } finally {
        ...
        <section className="w-full space-y-6 rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-md">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-relaxed text-slate-600">
              Enter the service access code supplied to your centre to open your
              Regulator Champions member resources.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            ...
            >
              {isSubmitting ? 'Checking access…' : 'Open Member Resources →'}
            </button>
          </form>

          <div className="space-y-3 border-t border-slate-100 pt-5">
            <p className="text-xs leading-relaxed text-slate-500">
              Access is provided to participating services. Keep your service
              code within your authorised educator team.
            </p>
...
export default function MemberAccessPage() {
  return (
    <Suspense fallback={<MemberAccessFallback />}>
      <MemberAccessContent />
    </Suspense>
  );
}